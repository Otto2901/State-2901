...
Deno.serve(async (req) => {
  ...
  const { alliance } = await req.json();
  if (!alliance) return new Response('OK');

  // Find reps for this alliance (exclude TRANSFER)
  ...
  const playerNames = reps.map((r: any) => r.player_name);
  ...
  const payload = JSON.stringify({
    ...
    body:  `${alliance}, it's your turn to select your Stronghold & Fort.`,
    tag:   'rotation-turn',
    url:   '/State-2901/#rotation'
  });
